...
[ "$(ip netns identify $$)" = "${PERF_NETNS:-smartdns-efficiency-20260912}" ] || {
    echo "Run with ip netns exec in the dedicated benchmark namespace" >&2; exit 1;
}
cd "${PERF_DIR:-/tmp/smartdns-efficiency-20260912}"
pid=; upstream=
cleanup() {
...
EOF
                taskset "$mask" ./oxidns start -d "$PWD" -c "$label.yaml" > "$label.stdout" 2>&1 &
            else
                cat > "$label.conf" <<EOF
bind 127.0.0.1:5300
